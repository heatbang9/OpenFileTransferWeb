...
- 사용자 TURN 서버 설정
- 로컬 전송 이력 저장
- PWA 설치와 앱 shell 오프라인 캐시
- 실패/중지 후 남은 파일 큐 재시도
- 동일 브라우저/동일 파일 메타데이터 기준 청크 단위 이어받기
...
- Service Worker는 앱 shell만 캐시하고 `/api/` signaling 요청은 캐시하지 않습니다.
- 초대/응답 코드를 제3자가 보면 연결 시도가 가능하므로 코드는 같은 사용자 흐름 안에서만 공유해야 합니다.

## 현재 제약

- 브라우저 탭이 닫히면 전송이 중단됩니다.
- `수신 파일 바로 저장`을 켜고 브라우저가 File System Access API를 지원하면 수신 청크를 사용자가 고른 파일로 바로 씁니다.
- File Picker가 없고 OPFS를 지원하는 브라우저는 OPFS 임시 파일에 청크를 쓰며, 같은 파일을 다시 보내면 이미 받은 청크를 건너뜁니다.
- 바로 저장을 사용할 수 없는 브라우저는 Blob으로 메모리에 모은 뒤 저장합니다.
- 이어받기는 파일 이름, 크기, 마지막 수정 시간, 청크 크기가 같은 경우에만 같은 파일로 판단합니다.
- 전송 암호를 사용한 파일은 재시도 시 같은 전송 암호를 입력해야 이어받을 수 있습니다.
- 매우 큰 파일은 브라우저별 저장 API와 메모리 제약을 받을 수 있습니다.
- NAT/방화벽 환경에 따라 직접 연결이 실패할 수 있습니다.
- TURN 서버 설정과 relay 후보 테스트는 UI로 제공하지만, 실제 전송 품질은 사용자가 입력한 TURN 서버 상태에 좌우됩니다.
...
3. 파일 해시 검증과 무결성 리포트
4. 이어받기 상태 정리 UI와 오래된 OPFS 임시 파일 청소
