...
- 별도 외부 방 서버나 사용자 계정 없이 Vercel 프로젝트 안에서 동작합니다.
...
2. Nearby 방에 참여해 같은 방의 디바이스에 `연결 요청`을 보내거나, `초대 코드 만들기`를 눌러 코드 또는 공유 링크를 받는 디바이스로 보냅니다.
3. 받는 디바이스에서 초대 코드를 붙여넣고, 같은 전송 암호가 있다면 입력한 뒤 `응답 코드 만들기`를 누릅니다.
4. 보내는 디바이스에서 응답 코드를 붙여넣고 `응답 코드 연결`을 누릅니다.
5. WebRTC 채널이 열리면 `선택 파일 전송`을 누릅니다.
6. 받는 디바이스의 `받은 파일` 목록에서 각 파일을 저장합니다.
...
- [Signaling, TURN, PWA, 재시도 설계](docs/signaling-and-reliability.md)
